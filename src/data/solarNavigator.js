...
const scoreLookup = (answers) => {
    let score = 50;
    if (answers.permission === "owner" || answers.permission === "approved")
        score += 8;
    if (answers.permission === "pending")
        score -= 6;
    if (answers.permission === "unknown")
        score -= 8;
    if (answers["roof-access"] === "high")
        score += 8;
    if (answers["roof-access"] === "medium")
        score += 4;
    if (answers["roof-access"] === "low")
        score -= 6;
    if (answers["roof-access"] === "none")
        score -= 10;
    if (answers.shading === "clear")
        score += 6;
    if (answers.shading === "heavy")
        score -= 6;
    if (answers.budget === "under500")
        score += 2;
    if (answers.budget === "500-2k")
        score += 4;
    if (answers.budget === "2k-8k")
        score += 6;
    if (answers.budget === "over8k")
        score += 8;
    if (answers.budget === "unsure")
        score -= 2;
    if (answers.timeline === "now" || answers.timeline === "soon")
        score += 4;
    if (answers.timeline === "later")
        score += 2;
    if (answers.timeline === "explore")
        score -= 2;
    if (answers.diy === "pro")
        score -= 2;
    if (answers.diy === "self-serve" || answers.diy === "guided")
        score += 2;
    return Math.max(0, Math.min(100, score));
};
const mapRisks = (answers) => {
    const risks = [];
    if (answers.permission === "pending" || answers.permission === "unknown")
        risks.push("Permissions unclear");
    if (answers["roof-access"] === "low" || answers["roof-access"] === "none")
        risks.push("Roof access caution");
    if (answers.shading === "heavy" || answers.shading === "unknown")
        risks.push("Confirm shading");
    if (answers.diy === "pro" || answers["safety-pref"] === "pro-lead")
        risks.push("Prefer certified installer");
    if (answers.budget === "unsure")
        risks.push("Clarify budget band");
    return risks;
};
const mapGuides = (answers) => {
    const guides = [];
    if (answers.goal === "bills")
        guides.push("navigator-warmup");
    if (answers.goal === "backup")
        guides.push("backup-basics");
    if (answers.goal === "eco")
        guides.push("workspace-playbook");
    if (answers["roof-access"] === "low" || answers["roof-access"] === "none")
        guides.push("rooftop-safety-basics");
    guides.push("maintenance-basics");
    return Array.from(new Set(guides));
};
const mapFirstStep = (answers) => {
    if (answers.permission === "pending" || answers.permission === "unknown")
        return "Focus on permissions and landlord approval first";
    if (answers.goal === "backup")
        return "Book a roof suitability check with outage priorities";
    if (answers["roof-access"] === "none")
        return "Start with a balcony or plug-in solar kit plan";
    if (answers.diy === "pro")
        return "Talk to a certified installer before wiring";
    return "Run a quick shade and budget validation";
};
const mapPathway = (answers) => {
    const steps = [];
    steps.push("Validate space and permissions with quick photos");
    if (answers.budget === "under500")
        steps.push("Model a starter kit with safe plug-in guidelines");
    else
        steps.push("Size rooftop array and shortlist hardware with safety limits");
    steps.push("Share plan with a helper or workspace and schedule safety review");
    return steps.slice(0, 3);
};
const mapTasks = (answers) => [
...
export const generateSolarPlan = (answers) => {
