...
import { addProjectGuideResource, fetchNotificationPreferences, fetchNotifications, fetchProjectDashboard, isBackendProjectId, setProjectGuidePinned, updateNotificationPreferences, updateNotificationRead, } from "@/lib/solaraApi";
...
function toResourceState(resources) {
...
function mergeResourceState(current, slug, pinned) {
...
const ProjectWorkspaceContainer = ({ project }) => {
    ...
    const volunteerPercent = useMemo(() => Math.min(100, Math.round((project.currentVolunteers / project.goalVolunteers) * 100)), [project.currentVolunteers, project.goalVolunteers]);
    ...
    const recommendedGuides = useMemo(() => recommendGuidesForProject(project)
        .map((slug) => guides.find((guide) => guide.slug === slug))
        .filter((guide) => Boolean(guide)), [project]);
    const searchableGuides = useMemo(() => guides.filter((guide) => {
    ...
    const visibleNotifications = useMemo(() => {
        ...
        const projectNotifications = notifications.filter((item) => item.projectId === backendProjectId ||
    ...
    const handleNotificationReadToggle = async (id, read) => {
        if (!backendProjectId)
            return;
        try {
    ...
    const addGuideToProject = async (slug) => {
        if (!backendProjectId) {
            const next = {
                ...resources,
                [project.id]: {
    ...
    return (_jsx(ProjectWorkspaceView, { project: project, tab: tab, onTabChange: setTab, tasks: tasks, onToggleTaskStatus: toggleTaskStatus, volunteerPercent: volunteerPercent, fundingPercent: fundingPercent, impactKwh: impactKwh, impactCO2: impactCO2, resourceError: resourceError, dialogOpen: dialogOpen, onDialogOpenChange: setDialogOpen, dialogContext: dialogContext, dashboardData: dashboardData, dashboardLoading: dashboardLoading, dashboardError: dashboardError, onRefreshDashboard: refreshDashboard, recommendedGuides: recommendedGuides, searchableGuides: searchableGuides, projectResources: projectResources, notifications: visibleNotifications, notificationsLoading: notificationsLoading, notificationsError: notificationsError, notificationPreferences: notificationPreferences, preferencesLoading: preferencesLoading, preferencesError: preferencesError, onRefreshNotifications: refreshNotifications, onToggleNotificationRead: handleNotificationReadToggle, onPreferenceChange: handlePreferenceChange, onSavePreferences: savePreferences, isSavingPreferences: isSavingPreferences, preferenceDirty: preferenceDirty, resourceCenterOpen: resourceCenterOpen, onOpenResourceCenter: () => setResourceCenterOpen(true), onCloseResourceCenter: () => setResourceCenterOpen(false), resourceSearch: resourceSearch, onResourceSearchChange: setResourceSearch, onAddGuideToProject: addGuideToProject, onAddGuideToTasks: addGuideToTasks, onTogglePin: togglePin, onPledgeHelp: () => {
            setDialogContext(`Pledge to help on ${project.name}`);
            setDialogOpen(true);
        }, onRequestHelpWithContext: () => {
            setDialogContext(`Request support for ${project.name}`);
            setDialogOpen(true);
        }, onRequestHelp: () => setDialogOpen(true), onHelpRequestSubmitted: async () => {
            await Promise.all([refreshDashboard(), refreshNotifications()]);
        } }));
};
