  ...
  return (
    <div className="rounded-[1.65rem] border border-[var(--solara-rule)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(246,249,255,0.92))] shadow-[var(--solara-shadow-soft)]">
      ...
      >
        ...
        <ChevronDown className={`h-5 w-5 text-[var(--solara-text-muted)] transition-transform ${isExpanded ? "rotate-180" : ""}`} />
      </button>
...
const StarterStat = ({ icon, label, value }) => (
...
const StarterCard = ({ eyebrow, title, body, meta, icon, cta, to, href }) => {
