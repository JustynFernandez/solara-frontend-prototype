...
function generatePresetPanels(preset, roof) {
    const panels = [];
    const spacing = 0.1;
...
export const useConfiguratorStore = create()(persist((set, get) => ({
    ...
    setSun: (config) => set((state) => ({
        sun: { ...state.sun, ...config },
    })),
    setSunPlayback: (config) => set((state) => ({
    ...
    setObstacleType: (type) => set({
        obstacleType: type,
    }),
    setSelectedPanel: (id) => set({
        selectedPanelId: id,
    }),
    reset: () => set({
        roof: defaultRoof,
