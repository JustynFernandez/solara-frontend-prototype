...
export function calculateEnergyEstimate(params: EstimateParams): EnergyEstimate {
  ...
  const systemEfficiency =
    SYSTEM_SPECS.inverterEfficiency * (1 - SYSTEM_SPECS.wiringLoss);
  ...
  const annualSavingsGbp =
    selfConsumedKwh * UK_ELECTRICITY_RATES.gridRate +
  ...
  const paybackYears =
    annualSavingsGbp > 0 ? totalSystemCost / annualSavingsGbp : 0;

  let lifetimeSavingsGbp = 0;
  for (let year = 0; year < SYSTEM_SPECS.systemLifeYears; year++) {
    const degradedOutput = Math.pow(
      1 - SYSTEM_SPECS.degradationPerYear,
      year
    );
...
export function useEnergyEstimate(params: EstimateParams): EnergyEstimate {
  return useMemo(
    () => calculateEnergyEstimate(params),
    [params.panelCount, params.orientation, params.pitchAngle, params.region, params.batteryCount]
  );
}
