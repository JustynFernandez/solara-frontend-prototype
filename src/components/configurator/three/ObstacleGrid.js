...
const ObstacleGrid = () => {
    const { obstacles, roof, toolMode, obstacleType, addObstacle, removeObstacle, } = useConfiguratorStore();
    const isRemoveMode = toolMode === "remove";
    const isObstacleMode = toolMode === "obstacle";
    const handleRoofClick = useCallback((e) => {
        if (!isObstacleMode)
            return;
        e.stopPropagation();
        const point = e.point;
        ...
        // Check bounds
        const inBounds = Math.abs(point.x) + spec.width / 2 <= halfWidth - 0.2 &&
        ...
        const hasOverlap = obstacles.some((obs) => {
            ...
            return (dx < (spec.width + obsSpec.width) / 2 + 0.2 &&
    ...
    const handleObstacleClick = useCallback((id) => (e) => {
        e.stopPropagation();
        if (isRemoveMode) {
            removeObstacle(id);
        }
    ...
    const clickableSurface = (_jsxs("mesh", { position: [0, roof.type === "flat" ? 0.16 : 0.5, 0], rotation: [-Math.PI / 2, 0, 0], onClick: handleRoofClick, visible: false, children: [_jsx("planeGeometry", { args: [roof.width, roof.depth] }), _jsx("meshBasicMaterial", { transparent: true, opacity: 0 })] }));
    const ObstacleComponent = {
        chimney: Chimney,
        skylight: Skylight,
        vent: Vent,
    };
    return (_jsxs("group", { children: [isObstacleMode && clickableSurface, obstacles.map((obstacle) => {
                const Component = ObstacleComponent[obstacle.type];
                return (_jsx(Component, { position: obstacle.position, rotation: obstacle.rotation, onClick: handleObstacleClick(obstacle.id), isRemoveMode: isRemoveMode }, obstacle.id));
            })] }));
};
