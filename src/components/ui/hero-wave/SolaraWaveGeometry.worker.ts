...
const addNormal = (normals: Float32Array, index: number, x: number, y: number, z: number) => {
  normals[index] += x;
  normals[index + 1] += y;
  normals[index + 2] += z;
};

const normalizeNormals = (normals: Float32Array) => {
...
const buildFoldedGeometry = (input: GeometryRequest): GeometryResponse => {
  const {
    width,
    height,
    subdivisionsX,
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    for (let column = 0; column < columns; column += 1) {
      ...
      const shoulderRidge =
        smoothstep(foldStart + 0.06, 0.72, u) *
...
workerScope.onmessage = (event: MessageEvent<GeometryRequest>) => {
