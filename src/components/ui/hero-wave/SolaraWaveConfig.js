...
const cloneVector3 = (value) => [value[0], value[1], value[2]];
const cloneSceneConfig = (config) => ({
    ...config,
    camera: {
        ...config.camera,
...
const getPerformanceConfig = (quality) => ({
    warmupSeconds: 10,
    lowFpsThreshold: quality === "low" ? 38 : 45,
    lowFpsWindowSeconds: 8,
    antialias: quality !== "low",
});
const applyQualityAdjustments = (scene, quality) => {
    if (quality === "high") {
        return scene;
    }
    if (quality === "medium") {
...
export const resolveWaveBreakpoint = (width) => {
    if (width < 640)
        return "mobile";
    if (width < 1264)
        return "tablet";
