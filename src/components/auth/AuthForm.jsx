...
const defaultAside = (
  <PreviewFrame
    chromeLabel="Account continuity"
    eyebrow="What carries through"
    title="Pick up the same work, the same context, and the same momentum."
    body="Sign in to resume planning, keep helper shortlists, and move between projects without rebuilding your setup."
...
const AuthForm = ({ title, subtitle, onSubmit, children, cta, aside = defaultAside, eyebrow }) => (
  <PageFrame family="hub" width="wide" density="compact" className="min-h-[calc(100vh-5rem)]">
    <PageReveal mode="mount">
      <FormShell
        eyebrow={eyebrow || cta || "Account"}
      title={title}
      body={subtitle}
      lead={
        <MetricBand
