...
const Navbar = ({ navItems, onSearchClick }) => {
  ...
  return (
    <div
      className={cn(
        "solara-navbar inset-x-0 z-40 px-4 sm:px-6 transition-[padding] duration-200 ease-out",
        isHeroImmersiveRoute ? "fixed top-0" : "sticky top-0",
        isHeroImmersiveRoute ? (compact ? "pb-0 pt-1" : "pb-0 pt-1.25") : "pb-0 pt-0"
      )}
    >
      <motion.nav
        ...
        className={cn(
          "relative mx-auto transition-[max-width] duration-200 ease-out",
          compact ? "max-w-[76rem]" : "max-w-7xl"
        )}
      >
        <NavbarSingleBar {...navbarProps} />
      </motion.nav>
    </div>
  );
};
