...
function resolveRouteFallback(projectId) {
  return (
...
const ProjectPreviewImage = ({ src, title }) => {
...
const ProjectDetailPage = () => {
  ...
  const fundingSummary =
    project.goalFunding && project.currentFunding != null
      ...
      : "Self-funded";
  const leadRoles = roles.slice(0, 3).map((role) => ({
    eyebrow: role.verified ? "Verified role" : "Team role",
    title: `${role.name} / ${role.userName}`,
    body: role.isHelper
      ? "This role is assigned to a helper account."
      : "This role is currently filled by a community member.",
  }));
      ...
      <PageReveal>
        <PageHeroStage
              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {tasks.slice(0, 4).map((task) => (
                    ...
                    >
                      <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--solara-accent-strong)]">
                        {task.status}
                      </p>
          ...
          metrics={[
            ...
            {
              label: "Impact",
              value: project.impactEstimateKwhPerYear
                ? `${project.impactEstimateKwhPerYear.toLocaleString()} kWh`
                : "Pending",
              meta: project.impactEstimateTonsCO2PerYear
                ? `${project.impactEstimateTonsCO2PerYear} tCO2 / yr`
                : "Estimate not set",
            },
          ]}
          rail={
            <ActionRail
              ...
              items={[
                {
                  eyebrow: "Funding",
                  title: fundingSummary,
                  body: "Current funding posture for the workspace.",
                },
                ...leadRoles,
              ]}
            />
